...
![HTTP2.0和HTTP1.1区别](images/HTTP2.0和HTTP1.1区别.png)

## 1、什么是HTTP 2.0

HTTP/2（超文本传输协议第2版，最初命名为HTTP 2.0），是HTTP协议的的第二个主要版本，使用于万维网。HTTP/2是HTTP协议自1999年HTTP 1.1发布后的首个更新，主要基于**SPDY协议**（是Google开发的基于TCP的[应用层](https://so.csdn.net/so/search?q=应用层&spm=1001.2101.3001.7020)协议，用以最小化网络延迟，提升网络速度，优化用户的网络使用体验）。
